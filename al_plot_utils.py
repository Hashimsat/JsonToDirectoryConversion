...
def bias_illustration(line_length, offset, optimal, y, ax, colors, center_x, rect_width, bias=np.nan, cue=np.nan):
    """ This function illustrates the decision biases (perseveration and environmental bias)

    :param line_length: Trial line length
    :param offset: Offset relative to center line
    :param optimal: Optimal choice position
    :param y: Y-axis position of lines
    :param ax: Axis handle
    :param colors: Plot colors
    :param center_x: X-axis position of central line
    :param rect_width: Width of choide symbol
    :param bias: Bias relative to optimal choice
    :param cue: Cue relative to optimal choice
    :return: ax, optimal_rect, bias_rect, cue_rect: Axis handle, and rectangle positions of optimal choice, bias,
     and cue
    """

    # Initialize rectangle arrays
    optimal_rect = np.full(3, np.nan)
    bias_rect = np.full(3, np.nan)
    cue_rect = np.full(3, np.nan)

    # Cycle over 3 trial examples
    for t in range(3):

        # Trial line
        ax.hlines(y[t], center_x - line_length - offset[t], center_x + line_length - offset[t], clip_on=False)

        # Compute x_0 of line and prediction bias
        line_x0 = center_x - line_length - offset[t]

        # Optimal prediction
        mu = line_x0 + optimal

        # Plot optimal choice rectangle
        optimal_rect[t] = mu-rect_width/2
        rect = plt.Rectangle((optimal_rect[t], y[t]-0.05), rect_width, 0.1, facecolor="k", alpha=1, zorder=2,
                             clip_on=False)
        ax.add_patch(rect)

        # Plot biased prediction rectangle
        if not np.isnan(np.sum(bias)):

            bias_rect[t] = mu + bias[t] - rect_width/2
            rect = plt.Rectangle((bias_rect[t], y[t] - 0.05), rect_width, 0.1, facecolor=colors[3], alpha=1, zorder=2,
                                 clip_on=False)
            ax.add_patch(rect)

        # Plot cue rectangle
        if not np.isnan(np.sum(cue)):
            cue_rect[t] = mu + cue[t] - rect_width/2

            rect = plt.Rectangle((cue_rect[t], y[t] - 0.05), rect_width, 0.1, facecolor="#92e0a9", alpha=1,
                                 zorder=2, clip_on=False)
            ax.add_patch(rect)

    return ax, optimal_rect, bias_rect, cue_rect


def dist_illustration(ax, line_length, offset, center_x, optimal, sigma, colors, bias, cue, rect_width):
    """ This function plots the distribution illustration
    ...
    :param cue: Cue relative to optimal choice
    :param rect_width: Width of choide symbol
    :return: ax, mu: Axis handle and mean position
    """

    # Initialize mean position
    mu = np.full(2, np.nan)

    for t in range(2):

        # Trial line
        ax.hlines(0.0, center_x[t] - line_length - offset, center_x[t] + line_length - offset, clip_on=False)

        # Compute x_0 of line and prediction bias
        line_x0 = center_x[t] - line_length - offset

        # Plot posterior distribution
        mu[t] = line_x0 + optimal
        x = np.linspace(mu[t] - 0.3, mu[t] + 0.3, 2001)
        x = np.round(x, 3)
        y = stats.norm.pdf(x, mu[t], sigma)
        y = y / 5
        ax.plot(x, y, color='k', alpha=1, clip_on=False)

        # Plot optimal choice rectangle
        rect = plt.Rectangle((mu[t] - rect_width/2, -0.05), rect_width, 0.1, facecolor='k', alpha=1, zorder=2,
                             clip_on=False)
        ax.add_patch(rect)

        # Plot bias rectangle
        if not np.isnan(bias[t]):

            rect = plt.Rectangle((mu[t]+bias[t] - rect_width/2, -0.05), rect_width, 0.1, facecolor=colors[3], alpha=1,
                                 zorder=2, clip_on=False)
            ax.add_patch(rect)

        # Plot cue rectangle
        if not np.isnan(cue[t]):
            rect = plt.Rectangle((mu[t] + cue[t] - rect_width / 2, -0.05), rect_width, 0.1, facecolor="#92e0a9",
                                 alpha=1, zorder=10, clip_on=False)
            ax.add_patch(rect)

    return ax, mu

def CircularDistance(angle1,angle2):
