...
def circular_distance(angle1,angle2):
    term1 = math.radians(angle1) - math.radians(angle2)
    sign_term1 = np.sign(term1)
    term2 = np.pi - abs(math.radians(angle1) - math.radians(angle2))
    sign_term2 = np.sign(term2)
    sign_overall = sign_term1*sign_term2

    return sign_overall*math.degrees(np.pi - abs(np.pi - abs(math.radians(angle1) - math.radians(angle2))))
